...
describe("StatementPrinter", () => {
  ...
  it("prints the statement header", () => {
    ...
    expect(consoleObj.printLine).toHaveBeenCalledWith(
      "Date || Amount || Balance"
    );
  });
  ...
  it("prints the transactions in chronological order", () => {
    ...
    expect(consoleObj.printLine.mock.calls[0][0]).toEqual(
      "Date || Amount || Balance"
    );
    expect(consoleObj.printLine.mock.calls[1][0]).toEqual(
      "14/01/2012 || -500 || 2500"
    );
    expect(consoleObj.printLine.mock.calls[2][0]).toEqual(
      "13/01/2012 || 2000 || 3000"
    );
    expect(consoleObj.printLine.mock.calls[3][0]).toEqual(
